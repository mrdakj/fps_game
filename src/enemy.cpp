...
void Enemy::set_idle() {
  auto global_transformation = m_skinned_mesh.get_bones_for_position("idle");
...
float Enemy::get_player_angle() const {
  ...
  float angle = (cross_product[1] > 0 ? 1 : -1) * glm::degrees(glm::angle(eye_direction, player_direction));
  return angle;
}
...
void Enemy::render(Shader &shader, Shader &bounding_box_shader,
  ...
  // BoundingBox(
  //     {100.0f * (get_gun_direction()), (gun_Y - gun_O), (gun_Z - gun_O)}, gun_O)
