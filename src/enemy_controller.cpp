...
void EnemyController::shoot() {
  ...
  if (fabs(gun_player_angle) < GUN_PLAYER_ANGLE_THRESHOLD) {
    if (!m_is_shooting) {
      m_is_shooting = on_animation_start(Animations::ShootStart);
    }
  } else if (m_is_shooting) {
    // should shooting end
    m_is_shooting = !on_animation_start(Animations::ShootEnd);
  }
}
...
void EnemyController::start_rotate_left() {
  bool start = on_animation_start(Animations::RotateLeft);
  if (start) {
    m_enemy.set_user_transformation(
        ...
        glm::inverse(m_enemy.get_final_global_transformation_for_animation(
            name(Animations::RotateLeft))));
  }
}

void EnemyController::start_rotate_right() {
  on_animation_start(Animations::RotateRight);
}
...
bool EnemyController::check_if_dead_and_animate() {
  ...
  if (is_shot) {
    on_animation_start(Animations::FallDead, true /* stop all active */);
